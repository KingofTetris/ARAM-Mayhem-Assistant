...
> 维护者：项目开发团队
> 变更记录：v1.9.0 完善 AD-018 代码注释规范（四层结构，中文注释，含公告模块实践）
...
注意：`areItemsTheSame` 中使用 `Objects.equals()` 而非 `==`，因为 ID 字段可能为 `Long` 包装类型（如 DTO 从 JSON 反序列化时）。若 ID 为 `long` 基本类型，`==` 即可；但 `Objects.equals()` 是通用安全写法，两种类型均适用。
...
**规则 AD-018**：代码注释规范（四层结构，中文注释）

所有 Java 源文件必须遵循以下四层注释规范，确保代码可维护性和团队协作效率。

**第一层：类级注释（Javadoc）**——每个类必须有
...
**要求**：
- **必须包含**：功能概述（一句话说明这个类是做什么的）
- **按类型补充**：
  - Fragment/Activity：需注明布局文件、导航关系
  - ViewModel：需注明数据流（来源→处理→输出）
  - Service：需注明核心业务逻辑
  - Repository：需注明数据源（远程+本地）
  - Entity/DTO/UiModel：需注明数据流向
  - Adapter：需注明数据源类型、布局文件、特性（如 DiffUtil）
  - 自定义 View：需注明使用场景
- **可选包含**：`@see` 引用关联类（帮助 IDE 导航和理解类间关系）
...
**第三层：方法级注释（Javadoc）**——公开方法和关键私有方法必须有
...
- 关键私有方法（如数据转换、缓存回退、异步回调逻辑）必须有 Javadoc
- 简单的 getter/setter/toString 可省略
- `@param` 必须说明参数含义和取值范围（如枚举值、null 含义）
- `@return` 必须说明返回值含义（成功/失败/null 的含义）
- 复杂方法需补充"作用"和"实现"说明
...
- 数据转换/映射必须注释说明转换规则
- 线程切换（主线程/子线程）必须注释说明
- **禁止**：不要对显而易见的代码加注释（如 `// 设置名字` 对 `name = "test"`）

**配置文件注释**——application.yml、build.gradle 等必须有注释

```yaml
spring:
  application:
    name: aram-server  # 应用名称，用于服务注册和日志标识
  datasource:
    url: jdbc:mysql://localhost:3306/aram_mayhem  # 数据库连接地址
...
**要求**：
- 所有配置项必须注释说明用途
- 敏感配置（密码、密钥）必须注明来源（环境变量/密钥服务）
- 非默认值必须注释说明原因

**豁免项**：
- Lombok `@Data` 生成的 getter/setter 不需要注释
- `@Override` 方法如果语义与父类一致可省略注释
- 测试类的方法名已自描述时可省略方法注释，但类注释仍需有
...
| 代码注释是否符合四层结构规范（类/字段/方法/行内） | AD-018 | 提交代码前 |
...
| 1.8.0 | 2026-05-16 | 新增 AD-018 代码注释规范（三层结构：类注释+方法注释+行内注释，中文注释，豁免项定义） | 项目开发团队 |
| 1.8.1 | 2026-05-16 | 建议修复项：S-04 AD-008 补充 Long/long 类型前提说明；S-06 BE-013 补充完整 CORS 配置示例；S-07 版本控制新增待修复规划 | 项目开发团队 |

### 待修复规划

| 目标版本 | 待修复项 | 优先级 |
|----------|---------|--------|
| v1.9.0 | M8 个人中心模块规则补充（ProfileViewModel 规范、SettingsFragment 规范） | P1 |
| v2.0.0 | M1-10 Checkstyle + SpotBugs 静态分析工具配置 | P2 |
| v2.0.0 | 全面代码注释合规性审计（基于 AD-018） | P2 |
